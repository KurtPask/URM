...
run_name="MARM-sudoku_loop16_H2_L6_layers4_gradaccum6_hidden512_head8_headsnormalized"
...
torchrun --nproc-per-node $SLURM_GPUS_ON_NODE --master_port=${MASTER_PORT} pretrain.py \
    ...
    arch=marm arch.loops=16 arch.H_cycles=2 arch.L_cycles=6 arch.hidden_size=512 arch.num_heads=8 arch.num_layers=4 \
    epochs=50000 \
    eval_interval=2000 \
    project_name=arcagi \
