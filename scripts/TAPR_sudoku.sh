...
project_name="${PROJECT_NAME:-tapr-sudoku}"
run_name="${RUN_NAME:-TAPR-sudoku-v3-ponder-nextlat-loop32-H1-L2-layers2-hidden768-head8}"
...
torchrun --nproc-per-node "${gpu_count}" --master_port="${MASTER_PORT}" pretrain.py \
    ...
    arch.L_cycles="${L_CYCLES:-2}" \
    arch.hidden_size="${HIDDEN_SIZE:-768}" \
    arch.num_heads="${NUM_HEADS:-8}" \
    arch.num_layers="${NUM_LAYERS:-2}" \
    arch.expansion="${EXPANSION:-4}" \
    arch.tropical_attention_version="${TROPICAL_ATTENTION_VERSION:-v3}" \
    arch.ponder_min_steps="${PONDER_MIN_STEPS:-3}" \
    arch.ponder_prior_lambda="${PONDER_PRIOR_LAMBDA:-0.14}" \
    arch.ponder_eval_threshold="${PONDER_EVAL_THRESHOLD:-0.58}" \
    arch.ponder_step_cost="${PONDER_STEP_COST:-0.001}" \
    arch.ponder_kl_weight="${PONDER_KL_WEIGHT:-0.01}" \
    arch.halt_correctness_weight="${HALT_CORRECTNESS_WEIGHT:-0.05}" \
    arch.nextlat_weight="${NEXTLAT_WEIGHT:-0.08}" \
    ...
    arch.chart_transition_strength="${CHART_TRANSITION_STRENGTH:-0.20}" \
    ...
    lr="${LR:-8e-4}" \
    puzzle_emb_lr="${PUZZLE_EMB_LR:-8e-4}" \
    ...
    wandb_tags='[tapr,sudoku,ponder,nextlat,tropical_attention]'
