...
def discover_latest_checkpoints(checkpoints_root: Path) -> List[Path]:
    ...
    for run_dir in sorted(p for p in checkpoints_root.iterdir() if p.is_dir()):
        ...
        if "hidden" not in run_name_l: #run_name_l != "urm-sudoku_h1_l32_layers1_bs100_gradaccum6":          #"tarm" in run_name_l:# and "v2" not in run_name_l: ### NOT DOING TARM FOR THIS RUN AT ALL
            continue
